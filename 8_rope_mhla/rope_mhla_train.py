...
def parse_args():
    ...
    parser.add_argument('--q_latent_dim',  type=int,   default=ModelConfig.q_latent_dim,    help='Latent Dimension of low rank Q space')
    parser.add_argument('--kv_latent_dim', type=int,   default=ModelConfig.kv_latent_dim,   help='Latent Dimension of low rank KV space')
    parser.add_argument('--rope_head_dim', type=int,   default=ModelConfig.rope_head_dim,   help='Number of dimensions of heads in RoPE')
...
def main(model:LLM, TrainingConfig:Trainconfig, ModelConfig:LLMconfig, optimizer:torch.optim.Optimizer):
    ...
    if TrainingConfig.save_model:
        torch.save(model, 'rope_mhla_llm_model.pt')
        print("\nsaved run to rope_mhla_llm_model.pt")
