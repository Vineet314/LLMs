...
class GQA(nn.Module):
    ...
    def forward(self, x:torch.Tensor, freqs_cis:torch.Tensor|None, kv_cache=None):
        ...
        # Apply RoPE
            q = utils.apply_rotary_emb(q, freqs_cis)
            k = utils.apply_rotary_emb(k, freqs_cis)
...
class FullMHLA(nn.Module):
    ...
    This class holds the weights, while the computation is done by the FullMHLA_triton kernel.
    """
    ...
    def forward(self, x:torch.Tensor, freqs_cis:torch.Tensor|None, kv_cache=None):
        # This forward pass now directly calls the Triton entrypoint function.
        return utils.FullMHLA_triton(self, x, freqs_cis, kv_cache)
